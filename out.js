...
;(() => {
            ...
            ;(function () {
                ...
                var runInContext = /* @__PURE__ */ __name(function runInContext2(context) {
                    context =
                        context == null
                            ? root
                            : _13.defaults(root.Object(), context, _13.pick(root, contextProps))
    ...
    // game/day_of_week.ts
    var all3 = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
    ...
    var _GameMap = class _GameMap {
        ...
        toString() {
            ...
            for (const y of _7.range(this.height)) {
                for (const x of _7.range(this.width)) s2 += repr(this.cells[y][x])
                s2 += "\n"
            }
            return s2
        }
    ...
    function directionTo(position, map, objType) {
        const dd = _7.compact(
            direction_exports.all.filter((x) => map.isAt(moveBy(position, x), objType)),
        )
    ...
    function getWallRepresentation(map, pos) {
        if (pos.x == 0 && pos.y == 0) {
            ...
            pos.y != 0 &&
            pos.y != map.height - 1 &&
            map.isAt(vector_exports.w(pos), "wall")
        ) {
            return "\u2562"
        } else if (
            pos.x == 0 &&
            pos.y != 0 &&
            pos.y != map.height - 1 &&
            map.isAt(vector_exports.e(pos), "wall")
        ) {
            return "\u255F"
            ...
            map.isAt(vector_exports.n(pos), "wall") &&
            map.isAt(vector_exports.s(pos), "wall")
        ) {
            return "\u2502"
    ...
    function useItem(player, item, map, effects) {
        ...
        switch (item.type) {
            ...
            case "coffee":
                ...
                player.flags.spedUp =
                    config_default.items.coffee.speedUpDays * config_default.dayTicks
                player.item = null
                return true
            case "door":
