...
;(() => {
    ...
    var __export = (target, all4) => {
        for (var name in all4) __defProp(target, name, { get: all4[name], enumerable: true })
    }
            ...
            ;(function () {
                ...
                var runInContext = /* @__PURE__ */ __name(function runInContext2(context) {
                    context =
                        context == null
                            ? root
                            : _14.defaults(root.Object(), context, _14.pick(root, contextProps))
                    ...
                    function basePullAll(array, values2, iteratee2, comparator) {
                        ...
                        while (++index < length) {
                            var fromIndex = 0,
                                ...
                                computed = iteratee2 ? iteratee2(value) : value
                            while (
                                (fromIndex = indexOf3(seen, computed, fromIndex, comparator)) > -1
                            ) {
                                if (seen !== array) {
                    ...
                    function isIndex(value, length) {
                        ...
                        return (
                            !!length &&
                            (type7 == "number" || (type7 != "symbol" && reIsUint.test(value))) &&
                            value > -1 &&
                            value % 1 == 0 &&
                            value < length
                        )
                    ...
                    var wrapperAt = flatRest(function (paths) {
                        var length = paths.length,
                            ...
                            interceptor = /* @__PURE__ */ __name(function (object) {
                    ...
                    function mixin(object, source, options) {
                        ...
                        var chain3 = !(isObject(options) && "chain" in options) || !!options.chain,
                    ...
                    arrayEach(["filter", "map", "takeWhile"], function (methodName, index) {
                        var type7 = index + 1,
                            isFilter = type7 == LAZY_FILTER_FLAG || type7 == LAZY_WHILE_FLAG
    ...
    // game/day_of_week.ts
    var all2 = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
    ...
    function generatePlan2(startTick) {
        ...
        const storySizes = ["small", "small", "small", "medium", "medium", "large"]
            ...
            for (const event of events) {
                switch (event.type) {
                    case "createBacklogIssue":
                        const story = story_exports.make(
                            game.map.getRandomEmptyLocation(),
                            event.size,
                        )
                        game.map.add(story)
                        append(effects, showMessage(`Moved "${story.name}" to To Do`, 500))
                        break
                    case "groomBacklogEnd":
                    case "collapseStart":
                        break
                    case "groomBacklogStart":
                        append(effects, showMessage("Grooming backlog ...", 2e3))
                        break
                    case "sprintDayEnd":
                        break
                    case "sprintDayStart":
                        ...
                        append(
                            effects,
                            showMessage(`Sprint day ${event.sprintDay} ${event.dayOfWeek}`, 3e3),
                        )
                        break
                    case "sprintEnd":
                        append(effects, showMessage("Sprint ended", 3e3))
                        const stories = objects_exports.filter(game.map.objects, "story")
                        game.map.remove(stories)
                        if (game.player.task != null) {
                            append(effects, showMessage(`Abandoned ${game.player.task}`, 2e3))
                    ...
                    case "workWeekEnded":
                        break
                    case "weekendStart":
                        append(effects, showMessage("Weekend, finally!!!", 3e3))
                        break
                    case "weekendEnd":
                        append(effects, showMessage("End of Weekend :(", 3e3))
                        break
                    case "gameEnded":
                    case "gameStarted":
                        break
    ...
    function generatePlan3(startDay) {
        ...
        for (const i in import_lodash4.default.range(
            Math.floor((config_default.totalDays - startDay) / 14),
        )) {
    ...
    function daily(plan2) {
        const result = [...plan2.entries()].map(([time, events]) => [
            Math.floor(time / config_default.dayTicks),
            events,
        ])
    ...
    function plan() {
        const plan2 = make2()
        addEvent(plan2, (config_default.totalDays - 5) * config_default.dayTicks, {
            type: "collapseStart",
        })
        return plan2
    }
    ...
    function tick5(itemGenerator, game) {
        switch (itemGenerator.state.type) {
            ...
            case "generating":
                if (
                    itemGenerator.state.tact > config_default.itemGenerator.interval &&
                    game.map.objects.filter(item_exports.isItem).length <
                        config_default.itemGenerator.maxItems
                ) {
    ...
    function directionTo(vector, target) {
        if (equals(vector, target)) return null
        else if (equals(n(vector), target)) return "up"
        else if (equals(s(vector), target)) return "down"
        else if (equals(w(vector), target)) return "left"
        else if (equals(e(vector), target)) return "right"
        else {
            return null
        }
    }
    ...
    function maze(size2, game) {
        const outer_walls = hline({ x: 0, y: 0 }, size2.x)
            .concat(vline({ x: 0, y: 0 }, size2.y))
            .concat(vline({ x: size2.x - 1, y: 0 }, size2.y))
            .map((point) => ({
                position: point,
                type: "wall",
                zIndex: 0,
        ...
        const inner_walls = import_lodash5.default
            .range(0, size2.y, 2)
            .map((y) => hline({ x: 0, y }, size2.x))
            .flatMap((x) => x)
            .map((point) => ({
                type: "wall",
                position: point,
    ...
    function roomWalls(args) {
        const room_walls = import_lodash5.default.flatMap(
            import_lodash5.default.range(3, args.height - 2, 2).map((y) =>
                import_lodash5.default.flatMap(
                    check(
                        () => ints(0, args.width, int(args.wallsPerRow.min, args.wallsPerRow.max)),
    ...
    function proper_distance(walls) {
        const notEmpty = /* @__PURE__ */ __name(
            (value) => value[0] !== null && value[1] != null,
            "notEmpty",
        )
        const sorted_walls = import_lodash5.default.sortBy(walls)
        const distances = import_lodash5.default
            .chain(import_lodash5.default.zip(sorted_walls, sorted_walls.slice(1)))
            .initial()
            .value()
            .filter(notEmpty)
    ...
    var _GameMap = class _GameMap {
        ...
        add(objs) {
            ...
            for (const obj of objs_) {
                if (obj.position != null) {
                    ...
                    objs2.push(obj)
                    this.cells[obj.position.y][obj.position.x] = _7
                        .chain(objs2)
                        .push(obj)
        ...
        someObjectsAt(v, include) {
            ...
            return objs.length > 0
                ? _7.some(objs, (x) =>
                      include instanceof Function ? include(x) : include == x.type,
                  )
        ...
        possibleDirections(position, check2) {
            ...
            for (const d of direction_exports.all) {
                ...
                if (this.everyObjectAt(newPos, check2)) p.push(d)
            }
            return p
        }
        toString() {
            ...
            for (const y of _7.range(this.height)) {
                for (const x of _7.range(this.width)) s2 += repr2(this.cells[y][x])
                s2 += "\n"
            }
            return s2
        }
        ...
        if (game.messages.length > 0) {
            ...
            if (Date.now() - game.messageStartTime.valueOf() > game.messages[0].ttl) {
                ...
                Date.now().valueOf() - game.messageStartTime.valueOf() >
                    config_default.messages.showNextMessageAfter &&
    ...
    function blink(a, b, tick13) {
        return tick13 % 10 < 5 ? a : b
    }
    __name(blink, "blink")
    function getRepresentation(map, objs, tick13) {
    ...
    function getWallRepresentation(map, pos) {
        if (pos.x == 0 && pos.y == 0) {
            ...
            pos.y != 0 &&
            pos.y != map.height - 1 &&
            map.someObjectsAt(vector_exports.w(pos), "wall")
        ) {
            return "\u2562"
        } else if (
            pos.x == 0 &&
            pos.y != 0 &&
            pos.y != map.height - 1 &&
            map.someObjectsAt(vector_exports.e(pos), "wall")
        ) {
            return "\u255F"
            ...
            map.someObjectsAt(vector_exports.n(pos), "wall") &&
            map.someObjectsAt(vector_exports.s(pos), "wall")
        ) {
            return "\u2502"
    ...
    function tick6(game) {
        const fullTick = /* @__PURE__ */ __name((timePassed) => {
            ...
            game.score.stockPrice =
                100 - (100 / config_default.totalDays) * (game.time.ticks / config_default.dayTicks)
    ...
    function objTick(obj, game, commands, ticksPassed) {
        switch (obj.type) {
            case "boss":
                ...
                break
            case "boss.footprint":
                handleEffects(game, boss_exports.Footprint.tick(obj, game.map))
                break
            case "developer.footprint":
                handleEffects(game, developer_exports.Footprint.tick(obj, game.map))
                break
            case "player":
    ...
    var _TextWindow = class _TextWindow extends Window {
        constructor(text) {
            ...
            this.contents = [topLine]
                .concat(splitText.map((x) => "\u2503 " + x + " \u2503"))
                .concat([bottomLine])
    ...
    // game/messages.ts
    var startedStory = /* @__PURE__ */ __name(
        (story) => ({
    ...
    var itemDropped = /* @__PURE__ */ __name(
        (item) => ({
    ...
    function possibleMoves(pos, currentDirection, map) {
        ...
        if (
            !import_lodash7.default.includes(possible, currentDirection) &&
                ...
                if (moves.back || moves.jump) {
                    const move_choice = choice(
                        import_lodash7.default.compact([
                            moves.back ? "back" : null,
                            moves.jump ? "jump" : null,
                        ]),
                        import_lodash7.default.compact([
    ...
    function pickDirection(target, map, pathway) {
        if (target.target == null || target.position == null) {
            const targets = map.objects
                .filter((x) => story_exports.isStory(x) || coffee_exports.isCoffee(x))
                .filter((x) => x.position != null)
            const paths = targets.map((x) => [x, findTarget(x, map, x.position, target.position)])
    ...
    function findTarget(obj, map, position, target) {
        return bfs(
            (v) => map.possibleDirections(v, (obj2) => obj2?.type != "wall"),
            position,
            target,
        )
    }
            ...
            switch (event.type) {
                ...
                case "workWeekEnded":
                    ...
                    effects_exports.append(
                        effects,
                        effect_exports.showMessage("Developer: have a nice weekend!", 1e3),
                    )
                    break
            }
        }
        ...
        switch (item.type) {
            case "commit":
                if (player.task) {
                    ...
                    effects_exports.append(
                        effects,
                        effect_exports.showMessage("No task to apply commit to", 3e3),
                    )
                    return false
                }
            case "coffee":
    ...
    function speedUp3(player) {
        speedUp_exports.speedUp(player, config_default.items.coffee.speedUpDays)
    }
    __name(speedUp3, "speedUp")
    function pickItem(player, newItem, game, effects) {
        ...
        switch (newItem.type) {
            case "door":
                ...
                player.item = newItem
                effects.push(effect_exports.showMessage(`Picked ${description(newItem)}`, 3e3))
                return true
            case "coffee":
                dropCarriedItem(player, game, effects)
                player.item = newItem
    ...
    function dropCarriedItem(player, game, effects) {
        const carriedItem = player.item
        if (carriedItem != null) {
            dropItem(player, game.map)
            effects_exports.append(
                effects,
                effect_exports.showMessage(`Dropped ${description(carriedItem)}`, 3e3),
            )
        }
    }
    ...
    function tickFlags(player, ticksPassed) {
        speedUp_exports.tick(player, ticksPassed)
    }
    ...
    function processCommands(player, commands, map, effects) {
        ...
        if (player.commands.length > 0) {
            logger4(JSON.stringify(player.commands))
        }
            ...
            switch (command.type) {
                case "move":
                    ...
                    if (canMoveOn(obsjAtNewPosition)) {
                        ...
                        logger4(`delaying ${JSON.stringify(delayedCommand)}`)
    ...
    function tick12(player, game, commands, ticksPassed) {
        const effects = []
                    ...
                    switch (obj.type) {
                        ...
                        case "coffee":
                            if (canPickItem(player, obj)) {
                                logger4(`Can pick item  ${JSON.stringify(player)}`)
                                effects_exports.append(
                                    effects,
                                    effect_exports.showMessage(`Picked a ${obj.type}`, 40),
                                )
                                pickedSomething =
    ...
    setIsEnabled((name) =>
        import_lodash11.default.includes(["main", "player", "game", "fellow_developer"], name),
    )
    function main() {
